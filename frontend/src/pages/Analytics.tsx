  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <p className="text-muted-foreground text-sm mt-1">Advanced insights and performance analysis</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card-elevated overflow-hidden">
          <div className="p-6 pb-3"><h3 className="text-sm font-semibold">Sponsor ROI Ranking</h3></div>
          <Table>
            ...
            <TableBody>
              {roiRanking.map(r => (
                <TableRow key={r.rank} className="hover:bg-muted/20 transition-colors">
                  <TableCell className="font-bold text-accent">{r.rank}</TableCell>
                  <TableCell className="font-medium">{r.sponsor}</TableCell>
                  <TableCell className="text-right font-semibold text-success">{r.roi}%</TableCell>
                  <TableCell className="text-right">{r.events}</TableCell>
                  <TableCell className="text-center">
                    <Badge variant="secondary" className={r.trend === 'up' ? 'bg-success/10 text-success' : r.trend === 'down' ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground'}>
        ...
        {/* Top Events */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card-elevated overflow-hidden">
          <div className="p-6 pb-3"><h3 className="text-sm font-semibold">Top Performing Events</h3></div>
          <Table>
            <TableHeader><TableRow className="bg-muted/30">
            ...
            <TableBody>
              {topEvents.map(e => (
                <TableRow key={e.name} className="hover:bg-muted/20 transition-colors">
                  <TableCell className="font-medium">{e.name}</TableCell>
                  <TableCell className="text-right"><Badge variant="secondary" className="bg-accent/10 text-accent">{e.score}</Badge></TableCell>
                  <TableCell className="text-right font-medium">${(e.revenue / 1000).toFixed(0)}K</TableCell>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card-elevated p-6">
          <h3 className="text-sm font-semibold mb-4">Year-over-Year Revenue</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={yoyData}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(220, 13%, 91%)" />
              <XAxis dataKey="quarter" tick={{ fontSize: 12, fill: 'hsl(220, 9%, 46%)' }} axisLine={false} tickLine={false} />
              ...
              <Bar dataKey="lastYear" fill="hsl(220, 13%, 85%)" radius={[4, 4, 0, 0]} name="Last Year" />
              <Bar dataKey="thisYear" fill="hsl(262, 83%, 58%)" radius={[4, 4, 0, 0]} name="This Year" />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="card-elevated p-6">
          <h3 className="text-sm font-semibold mb-4">Engagement vs Revenue Correlation</h3>
          <ResponsiveContainer width="100%" height={280}>
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(220, 13%, 91%)" />
              <XAxis dataKey="engagement" name="Engagement" tick={{ fontSize: 12, fill: 'hsl(220, 9%, 46%)' }} axisLine={false} tickLine={false} label={{ value: 'Engagement %', position: 'bottom', fontSize: 11, fill: 'hsl(220,9%,46%)' }} />
              <YAxis dataKey="revenue" name="Revenue" tick={{ fontSize: 12, fill: 'hsl(220, 9%, 46%)' }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}k`} />
              <ZAxis dataKey="z" range={[60, 400]} />
              <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid hsl(220,13%,91%)' }} formatter={(value: number, name: string) => [name === 'Revenue' ? `$${value}K` : `${value}%`, name]} />
              <Scatter data={scatterData} fill="hsl(262, 83%, 58%)" fillOpacity={0.7} />
            </ScatterChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
